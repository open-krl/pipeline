...
await check("bun:sqlite — schema.sql applies, WAL, STRICT, CHECK enforced", () => {
  ...
  const walPragma = sqlite.query("PRAGMA journal_mode;").get() as { journal_mode: string };
  expect(walPragma.journal_mode.toLowerCase() === "wal", "WAL not active");

  const fkPragma = sqlite.query("PRAGMA foreign_keys;").get() as { foreign_keys: number };
...
await check("drizzle-orm/bun-sqlite v1 — typed insert/select + constraints pass through", () => {
...
await check("src/db/connection.ts initDb — builds ephemeral DB and operates tables cleanly", () => {
  ...
  expect(found.length === 1 && found[0].sta_id === "BKS", "Failed stations query via initDb");

  sqlite.close();
  rmSync(dir, { recursive: true, force: true });
});

await check("zod 4 — safeParse shape, literal([0,1]), prettifyError, error param", () => {
  ...
  const t = z.string().regex(/^\d{2}:\d{2}:\d{2}$/, { error: "Expected HH:MM:SS" });
