...
| **17** | **Karet (`KAT`) $\to$ BNI City (`SUDB`) Operational Diversion Override**. | Rejecting the trip as a foreign key failure or mutating raw capture files. | On September 1, 2026, KAI Commuter temporarily suspended passenger boarding at Stasiun Karet (`KAT`) and diverted all 264 suburban Cikarang Line services to Stasiun BNI City (`SUDB`). Upstream updated 263 itineraries but missed train `5169D` (still emitting `KAT`), while omitting `KAT` from `stations.json`. Mapping `KAT` $\to$ `SUDB` in `STATION_CODE_OVERRIDES` aligns the unmigrated record with physical reality and the active dataset, compiling 100% of revenue trips cleanly (documented in `docs/anomalies/karet-bni-city-diversion.md`). |
...
| **Karet vs. BNI City Upstream Defect** | Check train `5169D` itinerary stops against active station catalog. | **Verified** ✅ | KAI Commuter Sept 1, 2026 diversion suspended Karet; unmigrated record emitted `KAT`. Resolved via `KAT` $\to$ `SUDB` in `STATION_CODE_OVERRIDES` (ADR 17). |
